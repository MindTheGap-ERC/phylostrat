...
if (FALSE){
  source("code/download_data.R") # download raw data if needed
}
...
repeat{
  ...
  if (class(tree_complete) == "phylo"){
    ...
    n_recent = tree_complete$tip.label[h > t_max - 10 * .Machine$double.eps] |> length()
    break
  }
}
...
recent_tips = tree_fp_complete$tip.label[h > t_max - 10 * .Machine$double.eps]
