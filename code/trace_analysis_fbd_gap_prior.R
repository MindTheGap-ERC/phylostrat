...
if (!dir.exists("figs/fbd_gap_prior")) {
  dir.create("figs/fbd_gap_prior", recursive = TRUE)
}
...
colnames = c("id", "nchar","ext", "spec", "or_time", "psi1", "psi2", "psi3", "psi4", "psi5", "br_rate_mol", "br_rate_morph", "kappa", "alpha")
...
for (id in ids){
  for (nchar in n_chars){
    ...
    psi1_est = comb_trace[[1]][["psi[1]"]] |> mean()
    psi2_est = comb_trace[[1]][["psi[2]"]] |> mean()
    psi3_est = comb_trace[[1]][["psi[3]"]] |> mean()
    psi4_est = comb_trace[[1]][["psi[4]"]] |> mean()
    psi5_est = comb_trace[[1]][["psi[5]"]] |> mean()
...
fig
ggsave("figs/fbd_gap_prior/fbd_gap_prior_summary.png", plot = fig)


df_psi = data.frame(matrix(nrow = 0, ncol = 3))
df_psi_names = c("psi", "nchar", "bin")
colnames(df_psi) = df_psi_names
for (i in 1:5){
  df_temp = data.frame(psi = df[[paste0("psi", i)]], nchar = df$nchar, bin = rep(paste0("bin", i), length(df$nchar)))
  df_psi = rbind(df_temp, df_psi)
}
df_psi$nchar = factor(df_psi$nchar, levels = c(30, 100, 300, 1000))
df_psi$bin = factor(df_psi$bin, levels = c("bin1", "bin2", "bin3", "bin4", "bin5"))

library(dplyr)
df_psi |>
  ggplot(aes(x = bin, y = psi, color = nchar)) +
  geom_boxplot() +
  ggtitle("Psi estimates for different sampling bins")
ggsave("figs/fbd_gap_prior/fbd_gap_prior_psi.png")
